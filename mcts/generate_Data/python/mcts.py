...
"""
- cada nó deve de guardar cada estado atual do jogo e as estatisticas acumuladas das das simulações
     - estado do jogo, tabuleiro (macro e micro)
     - num visitas
     - num_wins/num_losses/num_draws
     - estrutura da arvore: lista de childnodes para ligar as jogadas possiveis
"""
...
from game.ultimate_tic_tac_toe import UTTTGame, Action
...
class MonteCarloTreeSearch:
    ...
    def select_action(
        ...
        return Action(
            x=selected_evaluated_action["index"] % 9,
            y=selected_evaluated_action["index"] // 9,
            symbol=selected_evaluated_action["symbol"],
            index=selected_evaluated_action["index"],
        )

    def synchronize(self, uttt: UTTTGame) -> None:
...
def MonteCarloTreeSearchError(message: str) -> Exception:
    return Exception(f"MonteCarloTreeSearchError: {message}")



class Node:
    def __init__(self, uttt: UTTTGame, action: Optional[Action] = None):
        self.uttt = uttt
        self.action = action
        self.child_nodes = []
        self.num_visits = 0
    ...
    def is_terminal(self) -> bool:
        return self.uttt._check_game_over()
...
def dfs_max_depth(node: Node, depth: int) -> int:
    ...
    max_depth = depth
    for child_node in node.child_nodes:
        child_depth = dfs_max_depth(node=child_node, depth=depth + 1)
...
def playout(node: Node) -> Tuple[int, int, int]:
    ...
    while not uttt._check_game_over():
        actions = uttt.get_valid_actions()
        if not actions:
            break
        action = random.choice(actions)
        uttt.process_move(x=action.x, y=action.y)
    winner = uttt.who_is_winner()
    if winner == 1:
        num_X_wins += 1
    elif winner == 2:
        num_O_wins += 1
